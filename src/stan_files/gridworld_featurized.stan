data {
  int<lower=0> J;          // length of the reward vector weights
  ...
  matrix[m, 2] training_points;    // training dataset
  matrix[m, J] training_rewards;    // training dataset
  matrix[n, 2] behavior_points;          // expert demonstrations
}
parameters { // Correct
  vector<lower=-1.0, upper=1.0>[J] sample_reward;
}
transformed parameters { // calculate all the distance metrics.
  real likelihood = 0;
  real inner_sum = 0;
  real<lower=0.0> dist_rewards = 0;
  for (ii in 1:n) {
    dist_rewards = 0;
    for (jj in 1:m) {
